...
# 加了这行输出在运行过程中不出来，但是运行结束后会出来，应该是是buffer缓冲区的问题
...
class RuleDataset(Dataset):
    ...
    def __getitem__(self, idx):  # rule已经是通过模型生成的了，这里通过利用rule中body和置信度，还有facts，构造出了一个relation表，24个
        ...
        for rule in _rules:
            ...
            if conf_1 >=self.args.threshold:
                body_adj = sparse.eye(self.e_num)  # adj是邻接矩阵，创建一个稀疏矩阵，对角线上的元素为1，其余元素为0，表示实体之间没有关系的情况
                for b_rel in body:
                    body_adj = body_adj * self.r2mat[b_rel] # r2mat只是facts中实体的关系矩阵
                    # 将关系矩阵与r2mat中的实体关系矩阵相乘，得到当前规则的路径矩阵
                body_adj = body_adj * conf_1  # 要是置信度为0的话，是不是就相当于作废了
                path_count+=body_adj
        
        return rel, path_count

    # 这段代码是定义了一个静态方法collate_fn，用于将一个batch的数据进行拼接
    # 输入的data是一个列表，其中每个元素都是一个元组，包含了一个关系的路径矩阵和该关系的索引
    # 首先将每个元组中的索引和路径矩阵分别提取出来，然后将所有的索引和路径矩阵分别拼接成两个列表head和path_count，最后将这两个列表作为结果返回
    # 这个方法在DataLoader中被调用，用于将一个batch的数据进行拼接。这里batch_size设置的是1
    @staticmethod
    def collate_fn(data):
...
def construct_rmat(idx2rel, idx2ent, ent2idx, fact_rdf):  # 这里的fact_rdf是所有的三元组，fact_rdf+train_rdf+valid_rdf
    e_num = len(idx2ent)
    r2mat = {}
    # initialize rmat
    for idx, rel in idx2rel.items():
        mat = sparse.dok_matrix((e_num, e_num))  # 3000*3000的稀疏矩阵
        r2mat[rel] = mat
    # fill rmat
    for rdf in fact_rdf:
        fact = parse_rdf(rdf)
        ...
        r2mat[r][h_idx, t_idx] = 1  # family的话因为血缘关系只会两个实体只有一个关系，别的数据集是不是要考虑多关系
    return r2mat      
...
def kg_completion(rules, dataset, args):
    ...
    for q_i, query_rdf in enumerate(test_rdf):
        ...
        pred = np.squeeze(np.array(body2mat[q_r][ent2idx[q_h]]))  # 这里body2mat已经是使用过模型的结果了，只需要查表就行了，这里是合并一个batch，但是因为这里batch_size=1就没啥特殊的

        if pred[ent2idx[q_t]]!=0:
            pred_ranks = np.argsort(pred)[::-1]    
            
            truth = gt[(q_h, q_r)] # 原来给出h和r，需要预测t，但是正确答案t不是唯一的
            truth = [t for t in truth if t!=ent2idx[q_t]]
            # 从列表truth中删除等于ent2idx[q_t]的元素，truth是一个包含所有正确答案的列表，ent2idx[q_t]是当前查询的正确答案。这一行代码的作用是将当前查询的正确答案从正确答案列表中删除，以便在计算排名时不将其视为正确答案
            ...
            # 这里是需要把那些h，r相同的别的t删除，因为这些也是正确答案，但是不是当前查询的正确答案，所以需要删除
        ...
        print("rank at {}: {}".format(q_i, rank))  # 测试集中的第几个，rank是多少
...
if __name__ == "__main__":
    ...
    parser.add_argument("--threshold", type=float, default=0)  # 为0的话后面代码会乘上置信度，就相当于没用了
    parser.add_argument("--rule_num", type=int, default=500)
    parser.add_argument("--use_rule_len", type=int, default=2)
    args = parser.parse_args()
    dataset = Dataset(data_root='../datasets/{}/'.format(args.data), inv=True)
    # dataset = Dataset(data_root='../datasets/{}/'.format(args.data), inv=False)
    all_rules = {}
    all_rule_heads = []
   
    # for L in range(2,3):  # range是左闭右开的，所以这里只有2和3
    ...
    # 在这里调整使用的规则长度
    for L in range(2, args.use_rule_len+1):
        #file = "{}/{}_500_{}.txt".format(args.rule, args.rule, L)
        file = "{}/{}_{}_{}.txt".format(args.rule, args.rule, args.rule_num, L)
        load_rules("{}".format(file), all_rules, all_rule_heads)

    for list_ in all_rules.values():
